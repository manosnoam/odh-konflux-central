...
def pipeline_run_name_from_env() -> str:
...
def get_pipelinerun_json(pipeline_run: str, namespace: str) -> dict[str, Any]:
    ...
    try:
        return _k8s_request("GET", url, token, ca_path)
...
def task_result(taskruns: list[dict[str, Any]], task_substr: str, result_name: str) -> str:
    ...
    for tr in taskruns:
        if needle not in _task_name(tr).lower():
            continue
        val = _result_map(tr).get(result_name, "").strip()
        if val:
            return val
    return ""
...
def pipeline_succeeded_status_label(prj: dict[str, Any]) -> str:
    cond = next((c for c in (prj.get("status") or {}).get("conditions", []) if c.get("type") == "Succeeded"), {})
