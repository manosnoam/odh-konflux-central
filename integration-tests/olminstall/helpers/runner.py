...
class OLMInstallRunner:
    ...
    def wait_for_pipelinerun(self) -> None:
        ...
        wait_stream = _interactive_progress_stream()
        # Empty TERM is common in embedded terminals; only ``dumb`` is treated as non-ANSI.
        term = (os.environ.get("TERM") or "").strip()
        use_spinner = wait_stream.isatty() and term != "dumb"
        spin_frames = "|/-\\"
        wait_stream.write(msg_prefix)
        wait_stream.flush()
        ...
        try:
            for attempt in range(1, attempts + 1):
                if use_spinner:
                    ch = spin_frames[(attempt - 1) % len(spin_frames)]
                    wait_stream.write(f"\r\033[K{msg_prefix} {ch}")
                    wait_stream.flush()
                items = self.get_pipelineruns(self.args.namespace)
                cands = []
                for item in items:
                ...
                if cands:
                    ...
                    self.pr = cands[-1][1]
                    if use_spinner:
                        wait_stream.write(f"\r\033[K{msg_prefix} ok")
                        wait_stream.flush()
                    wait_stream.write("\n")
                    wait_stream.flush()
                    finished_ok = True
            ...
            if use_spinner and not finished_ok:
                wait_stream.write("\r\033[K")
                wait_stream.flush()
                wait_stream.write("\n")
                wait_stream.flush()
