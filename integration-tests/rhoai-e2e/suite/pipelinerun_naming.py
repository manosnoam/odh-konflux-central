...
_LEGACY_OLMINSTALL_PLR_PREFIX = "olminstall"
...
def is_rhoai_e2e_pipelinerun_name(name: str) -> bool:
    ...
    if not text:
        return False
    return (
        text.startswith(f"{_E2E_PLR_PREFIX}-")
        or text.startswith(f"{_LEGACY_PLR_PREFIX}-")
        or text.startswith(f"{_LEGACY_OLMINSTALL_PLR_PREFIX}-")
    )
